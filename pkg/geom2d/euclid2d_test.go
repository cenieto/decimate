...
func TestCrossProduct2D(t *testing.T) {
	...
	for values := range lines {
		...
		v1 := primitives.NewVector([]float64{values[0], values[1]}) // First vector for cross product
		v2 := primitives.NewVector([]float64{values[3], values[4]}) // Second vector for cross product
